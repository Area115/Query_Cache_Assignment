...
| -------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `main.py`            | Core cache and plan manager          | - Manages query normalization and plan caching<br>- Handles plan generation, reuse, and cache metrics (hits/misses/requests)                              |
| `walker_listener.py` | Query normalization using ANTLR      | - Implements ANTLR Walker–Listener traversal<br>- Extracts literals, leaf subqueries, and masked outer queries<br>- Replaces values with `?` placeholders |
| `test_main.py`       | Performance testing and benchmarking | - Runs multiple SQL queries (simple & nested)<br>- Simulates planning delays via complexity scores<br>- Compares with/without cache performance           |
| `grammer/SQLite.g4`  | SQL grammar definition (ANTLR input) | - Defines lexical and syntactical structure of SQL used by ANTLR to generate lexer, parser, listener, and visitor files                                   |
| `show_plan.py`       | Output formatter                     | - Displays query plans, literals, and cache statistics in a structured, human-readable format                                                             |
| `Output.txt`         | Output log                           | - Stores final normalized queries, plans, and cache performance metrics                                                                                   |


**Command to generate Lexar , Parser , Visitor and Listner files :**

```python

java -Xmx500M -cp "C:\tools\antlr\antlr-4.13.1-complete.jar" org.antlr.v4.Tool ^
-Dlanguage=Python3 -visitor -listener -o grammer grammer/SQLiteLexer.g4 grammer/SQLiteParser.g4
